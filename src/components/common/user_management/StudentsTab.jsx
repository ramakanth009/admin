// src/components/common/user_management/StudentsTab.jsx
...
/**
 * Unified students tab component that displays student data based on user role
 * 
 * @param {Object} props - Component props
 * @param {string} props.role - User role ('college_admin' or 'department_admin')
 */
const StudentsTab = ({ role }) => {
  // Use Auth context to detect role if not provided in props
  const { userRole, isCollegeAdmin } = useAuth();
  const effectiveRole = role || userRole || 'college_admin';
  const colors = getPrimaryColors(effectiveRole);
  ...
  const [studentFilters, setStudentFilters] = useState(
    isCollegeAdmin() ? 
      { department: '', isActive: '', profileCompleted: '' } :
      { preferredRole: '', isActive: '', profileCompleted: '', canUpdateProfile: '' }
  );
  ...
  const fetchStudents = useCallback(async (silentRefresh = false) => {
    try {
      ...
      try {
        const response = await apiService.admin.getStudents(studentPage, studentFilters);
        ...
        setStudents([
          {
            ...
            preferred_role: isCollegeAdmin() ? null : 'software_developer',
            can_update_profile: isCollegeAdmin() ? null : false
          },
          {
            id: 2,
            username: 'student002',
            ...
            preferred_role: isCollegeAdmin() ? null : 'data_analyst',
            can_update_profile: isCollegeAdmin() ? null : true
          }
        ]);
  ...
  }, [studentPage, studentFilters, isCollegeAdmin]);
      ...
      try {
        ...
        setUserDetails({
          profile: {
            ...
            [isCollegeAdmin() ? 'department_rank' : 'rank_in_department']: '5/30'
          },
          academic_statistics: {
            total_assessments: 5,
    ...
    <>
      ...
      <Box className={classes.titleContainer}>
        <Typography variant="h5" className={classes.sectionTitle}>
          {isCollegeAdmin() ? 'Student Management' : 'Department Students'}
        </Typography>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              {/* Department column for college admin only */}
              {isCollegeAdmin() && <TableCell>Department</TableCell>}
              
              {/* Role preference for department admin only */}
              {!isCollegeAdmin() && <TableCell>Role Preference</TableCell>}
          ...
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell colSpan={isCollegeAdmin() ? 8 : 8} align="center" sx={{ py: 3 }}>
                ...
                >
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <SchoolIcon sx={{ mr: 1, color: colors.main }} />
                      {student.username}
                    </Box>
                  </TableCell>
                  ...
                  <TableCell><StatusChip status={student.is_active} type="active" role={effectiveRole} /></TableCell>
                  <TableCell><StatusChip status={student.profile_completed} type="profile" role={effectiveRole} /></TableCell>
                  <TableCell>
